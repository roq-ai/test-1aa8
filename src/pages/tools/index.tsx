...
export function ToolListPage(props: ToolListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getTools({
        relations: ['api_key', 'customer_tool_usage.count'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('api_key', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'api_key',
          header: 'Api Key',
          accessorKey: 'api_key',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/api-keys/view/${record.api_key?.id}`}>
              {record.api_key?.key_value}
            </Link>
          ),
        }
      : null,
    hasAccess('customer_tool_usage', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'customer_tool_usage',
          header: 'Customer Tool Usage',
          accessorKey: 'customer_tool_usage',
          cell: ({ row: { original: record } }: any) => record?._count?.customer_tool_usage,
        }
      : null,
